...
module.exports.execute = async function(interaction, client, userId) {
  ...
  if (content.length && embed.length) await db.child(guild.id).child("gb").child("m").set({
    content: content,
    embeds: embed[0]
  });
  if(content.length != 0 && embed.length === 0) await db.child(guild.id).child("gb").child("m").set({
    content: content
  });
  if (content.length == 0 && embed.length != 0) await db.child(guild.id).child("gb").child("m").set({
    embeds: embed[0]
