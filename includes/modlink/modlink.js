...
module.exports = {
  async modlink(interaction, client, args) {
  ...
  if (interaction.user.id != userId) return interaction.reply(ephemeral("⚠️ Maaf, tapi kamu gak ada izin untuk itu."));
    const files = fs.readdirSync(__dirname).filter(file => file.endsWith('.js') && file != "modlink.js");
    const event = require(path.join(__dirname, files[files.findIndex(n=> n === args.split("_")[0]+".js")]))
    return event.execute(interaction, client, userId, args.split("_"));
  }
}