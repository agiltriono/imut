...
class Embed {
  constructor(element, bot = {}) {
    this.element = element;
    this.bot = {avatar: bot.avatar,username:bot.username}
  }
  html () {
    const bot = this.bot
    return `<div class="msgEmbed">
        <div class="mcontent">
            <img src="${bot.avatar ? bot.avatar : $(this.element).attr("avatar") ? $(this.element).attr("avatar") : "https://cdn.discordapp.com/embed/avatars/1.png"}" class="avatar" alt=" " />
            <h2>
                <span class="username" role="button">${bot.username ? bot.username : $(this.element).attr("username") ? $(this.element).attr("username") : "Shayang"}</span>
                <span class="botTag">
                ...
                <span class="timeText">${this.timestamp()}</span>
            </h2>
        </div>
  ...
  createEmbedFields (fields, embedFields) {
            ...
            if (!f.inline)
                fieldElement.outerHTML = `
                    <div class="embedField" style="grid-column: 1 / 13;">
                        <div class="embedFieldName">${this.markup(this.encodeHTML(f.name), { inEmbed: true, replaceEmojis: true, inlineBlock: true })}</div>
                        <div class="embedFieldValue">${this.markup(this.encodeHTML(f.value), { inEmbed: true, replaceEmojis: true })}</div>
                    </div>`;
            else {
    ...
    this.display(embedFields, undefined, 'grid');
  }
  buildEmbed (jsonData) {
    var embedContent = document.querySelector('.messageContent')
    var embedCont = document.querySelector('.msgEmbed > .contain')
    $(this.element).find(".messageContent").addClass("emptyContent")
    $(this.element).find(".embed").addClass("emptyEmbed")
    try {
        // If there's no message content, hide the message content HTML element.
        if (jsonData.content) {
          embedContent.innerHTML = this.markup(this.encodeHTML(jsonData.content), { replaceEmojis: true });
          $(this.element).find(".messageContent").removeClass("emptyContent")
        }
        if(jsonData.embeds) {
          $(this.element).find(".embed").removeClass("emptyEmbed")
        }
        if (!jsonData.embeds) return;
        const embedObj = jsonData.embeds
            const embedElement = embedCont;
            ...
            if (embedObj.title) this.display(embedTitle, this.markup(`${embedObj.url ? '<a class="anchor" target="_blank" href="' + this.encodeHTML(this.url(embedObj.url)) + '">' + this.encodeHTML(embedObj.title) + '</a>' : this.encodeHTML(embedObj.title)}`, { replaceEmojis: true, inlineBlock: true }));
            else this.hide(embedTitle);

            if (embedObj.description) this.display(embedDescription, this.markup(this.encodeHTML(embedObj.description), { inEmbed: true, replaceEmojis: true }));
            else this.hide(embedDescription);
            ...
            if (embedObj.author?.name) this.display(embedAuthor, `
                ${embedObj.author.icon_url ? '<img class="embedAuthorIcon embedAuthorLink" src="' + this.encodeHTML(this.url(embedObj.author.icon_url)) + '">' : ''}
                ${embedObj.author.url ? '<a class="embedAuthorNameLink embedLink embedAuthorName" href="' + this.encodeHTML(this.url(embedObj.author.url)) + '" target="_blank">' + this.encodeHTML(embedObj.author.name) + '</a>' : '<span class="embedAuthorName">' + this.encodeHTML(embedObj.author.name) + '</span>'}`, 'flex');
            else this.hide(embedAuthor);
            ...
            if (embedObj.thumbnail?.url) {
                ...
                this.hide(embedThumbnail.parentElement);
            ...
            else this.hide(embedImage.parentElement);

            if (embedObj.footer?.text) this.display(embedFooter, `${embedObj.footer.icon_url ? '<img class="embedFooterIcon embedFooterLink" src="' + this.encodeHTML(this.url(embedObj.footer.icon_url)) + '">' : ''}<span class="embedFooterText">
                    ${this.encodeHTML(embedObj.footer.text)}
                ${embedObj.timestamp ? '<span class="embedFooterSeparator">•</span>' + this.encodeHTML(this.timestamp(embedObj.timestamp)) : ''}</span></div>`, 'flex');
            else if (embedObj.timestamp) this.display(embedFooter, `<span class="embedFooterText">${this.encodeHTML(this.timestamp(embedObj.timestamp))}</span></div>`, 'flex');
            else this.hide(embedFooter);

            if (embedObj.fields) this.createEmbedFields(embedObj.fields, embedFields);
            else this.hide(embedFields);
    } catch (e) {
        throw Error(e);
    }
  }
